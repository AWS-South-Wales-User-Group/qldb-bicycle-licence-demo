...
const { getContactHistory } = require('./helper/licence');
...
const handler = async (event) => {
  ...
  logger.debug(`In the get-contact-history handler with licenceId ${licenceid}`);

  try {
    const response = await getContactHistory(logger, licenceid);
